...
function UpcomingExams({ attempts }: { attempts: Attempt[] | null }) {
    ...
    // Get exam IDs where student has completed attempts
    ...
    // Filter to only show exams that haven't been completed
                    ...
                    <Card key={exam.id} className="bg-card/80 backdrop-blur-sm">
                        ...
                        <CardFooter className="flex gap-2">
                            <Button 
                                variant="outline" 
                                className="flex-1" 
                                onClick={() => router.push(`/exam/${exam.id}/results?attemptId=${latestAttempt?.id}`)}
                            >
                                View Results
                            </Button>
                            <Button 
